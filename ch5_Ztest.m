...
ShowGD('坐标轮转法',matlabFunction(f),x_list,y_list);hold on;

%% Hooke-Jeeves算法: X_list的长度比坐标轮转法更少
X_list = ch5_HooJee(f, X0, lb, ub, epsilon);
x_list = X_list(1,:);y_list = X_list(2,:);
ShowGD('Hooke-Jeeve法',matlabFunction(f),x_list,y_list);hold on;
...
ShowGD('Nelder-Mead Simplex法',matlabFunction(f),x_list,y_list);hold on;

%% DIRECT
[X_list,P_list,m,t] = ch5_MultiDIRECT(f, lb, ub, -4, epsilon);  
% 如果不给定函数的最优值-4作为判别收敛条件，则收敛速度很慢
x_list = X_list(1,:);y_list = X_list(2,:);    
ShowGD('DIRECT法',matlabFunction(f),x_list,y_list);hold on;
% % 绘制被评估过的点位置
